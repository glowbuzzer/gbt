...
class go_hom implements Igo_hom {
    ...
    constructor(go_cart: go_cart = {x: 0, y: 0, z: 0}, go_mat: go_mat = {
        x: {x: 0, y: 0, z: 0},
        y: {x: 0, y: 0, z: 0},
        z: {x: 0, y: 0, z: 0}
    }) {
...
class go_pk implements Igo_pk {
    ...
    platform: go_cart 		/*!< position of moving end in platform frame  */
    d: number			/*!< the length of the link */
    constructor(base: go_cart = {x: 0, y: 0, z: 0}, platform: go_cart = {x: 0, y: 0, z: 0}, d: number = 0) {
        this.base = base;
...
class go_urdf implements Igo_urdf {
    pose: go_pose 	/*!< the pose of the link wrt to the previous link */
    axis: go_cart		/*!< the axis of rotation or translation */
    constructor(pose: go_pose = {tran: {x: 0, y: 0, z: 0}, rot: {s: 1, x: 0, y: 0, z: 0}}, axis: go_cart = {
        x: 0,
        y: 0,
        z: 0
    }) {
...
class go_body implements Igo_body {
    ...
    constructor(mass: number = 0, inertia: number[][] = [[0, 0, 0], [0, 0, 0], [0, 0, 0]]) {
        this.mass = mass;
...
class go_pose implements Igo_pose {
    tran: go_cart
    rot: go_quat
...
class go_mat implements Igo_mat {
    ...
    constructor(x: go_cart = {x: 0, y: 0, z: 0}, y: go_cart = {x: 0, y: 0, z: 0}, z: go_cart = {x: 0, y: 0, z: 0}) {
        this.x = x;
        this.y = y;
        this.z = z;
    }
...
export class go_link implements Igo_link {
    ...
    type: LinkParamRepresentation		       /*!< one of GO_LINK_DH,PK,PP  */
    quantity: LinkQuantities	       /*!< one of GO_QUANTITY_LENGTH,ANGLE */
    constructor(dh: go_dh = {a: 0, alpha: 0, d: 0, theta: 0}, pk: go_pk = {
        base: {x: 0, y: 0, z: 0},
    ...
    }, type: LinkParamRepresentation = LinkParamRepresentation.GO_LINK_DH, quantity: LinkQuantities = LinkQuantities.GO_QUANTITY_LENGTH) {
        this.dh = dh;
        this.pk = pk;
        this.pp = pp;
        this.urdf = urdf;
...
class go_matrix implements Igo_matrix {
    rows: number
    cols: number
    el: number[][]
    elcpy: number[][]
    ...
    constructor(rows: number = 0, cols: number = 0, el: number[][] = [], elcpy: number[][] = [], v: number[] = [], index: number[] = []) {
        this.rows = rows;
        this.cols = cols;
        this.el = el;
        this.elcpy = el;
...
function go_mat_is_norm(m: go_mat): boolean {
    ...
    return (COL_IS_UNIT(m.x) && COL_IS_UNIT(m.y) && COL_IS_UNIT(m.z) && Boolean(go_cart_cart_compare(vout, m.z)));
}
...
export function go_matrix_vector_cross(a: go_matrix, v: go_vector): { ret: number, axv: go_matrix } {
    ...
    if (a.rows != 3 || axv.rows != 3 || a.cols != axv.cols) return {ret: retval.GO_RESULT_ERROR, axv: null};
...
//     if (0 == m.el[0] || 0 == minv->el[0]) return GO_RESULT_ERROR;
    ...
    for (let col: number = 0; col < N; col++) {
        ...
        const retval = lubksb(m.elcpy, N, m.index, m.v);
